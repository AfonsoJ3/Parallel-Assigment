...
int main(int argc, char* argv[])
{
    ...
    MPI_Reduce(&(Bend - Bstart) , totalB , 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce(&(Bend_m - Bstart_m) , totalB_m, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce(&(calEnd - calStart) , totalcal , 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce(&(gEnd - gEnd) , totalg , 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0)
    {
        ...
        printf("The Bcast with %d ranks, took %.10f seconds", numranks, totalB);
        printf("The calculation with %d ranks, took %.10f seconds", numranks, totalcal);
        printf("The gather with %d ranks, took %.10f seconds", numranks, totalg);
